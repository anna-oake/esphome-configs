...
struct HIDReportItemValue {
  HIDReportItemValue(const HIDUsage usage, const int32_t value,
...
public:
  HIDUsageRange(const HIDUsage usage_min, const HIDUsage usage_max,
                uint16_t usage_page)
...
public:
  HIDInputReportItem(HIDUsageCollection *usage_collection, uint8_t report_count,
                     uint8_t report_id, HIDLogicalRange logical_range,
                     uint8_t report_size, uint8_t report_offset)
      : report_size(report_size), usage_collection(usage_collection),
        report_count(report_count), report_id(report_id),
        logical_range(logical_range), report_offset(report_offset) {
    this->last_values = std::vector<HIDReportItemValue>(
        report_count, HIDReportItemValue(HIDUsage(0, 0), 0, 0));
  }
  virtual ~HIDInputReportItem() { delete usage_collection; };
  size_t get_total_size();
  virtual std::vector<HIDReportItemValue> parse(uint8_t *hid_report_data) = 0;
  static int32_t parse_input_report_item(uint8_t *report_data,
                                         uint16_t bit_offset,
...
public:
  ...
  static HIDReportMap *parse_report_map_data(const uint8_t *report_map_data,
                                             uint16_t report_map_size);
  static int32_t parse_item(const uint8_t **report_map_data,
                            uint16_t *report_map_size,
