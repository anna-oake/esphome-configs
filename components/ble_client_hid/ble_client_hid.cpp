...
void BLEClientHID::loop() {
  ...
  case HIDState::BLE_CONNECTED:
    this->read_client_characteristics(); // not instant, finished when hid_state
                                         // = HIDState::READ_CHARS
...
void BLEClientHID::gattc_event_handler(esp_gattc_cb_event_t event,
  ...
  case ESP_GATTC_READ_DESCR_EVT: {
    ...
    GATTReadData *data = new GATTReadData(param->read.handle, param->read.value,
...
void BLEClientHID::send_input_report_event(esp_ble_gattc_cb_param_t *p_data) {
  ...
  for (HIDReportItemValue value : hid_report_values) {
    std::string usage = std::to_string(value.usage.page) + "_" +
                        std::to_string(value.usage.usage);
    auto btn = (GamepadButton)value.usage.usage;
    if (btn == GamepadButton::STICK_X || btn == GamepadButton::STICK_Y) {
      if (btn == GamepadButton::STICK_X) {
        this->stick_x = value.value;
      } else {
        this->stick_y = value.value;
      }
      auto cmd = convertXYtoTank(this->stick_x, this->stick_y);
      for (auto &trigger : this->stick_triggers_)
        trigger->trigger(cmd.angle, cmd.velocity);
      ESP_LOGD(TAG, "angle: %f, velocity: %f", cmd.angle, cmd.velocity);
      continue;
    }

    std::string name = "";
    switch ((GamepadButton)value.usage.usage) {
    ...
    for (auto &trigger : this->key_triggers_)
      trigger->trigger(name, value.value == 1);
    ESP_LOGD(TAG, "%s = %d", name.c_str(), value.value);
  }
...
void BLEClientHID::register_stick_trigger(GamepadStickTrigger *trig) {
  this->stick_triggers_.push_back(trig);
}

void BLEClientHID::register_key_trigger(GamepadKeyTrigger *trig) {
...
StickCmd convertXYtoTank(uint32_t Xraw, uint32_t Yraw) {
  ...
  float horiz = (static_cast<int>(Yraw) - CENTER) / SCALE;
  float vert = (CENTER - static_cast<int>(Xraw)) / SCALE;

  if (std::fabs(horiz) < deadzone)
    horiz = 0.0f;
  if (std::fabs(vert) < deadzone)
    vert = 0.0f;

  StickCmd out;
  out.angle = -clamp(horiz, -1.0f, 1.0f) * 120.0f;
  if (out.angle == 0.0f && std::signbit(out.angle))
    out.angle = 0.0f;
  out.velocity = clamp(vert, -1.0f, 1.0f);
  return out;
}
