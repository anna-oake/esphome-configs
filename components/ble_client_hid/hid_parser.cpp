...
int32_t HIDReportMap::parse_item(const uint8_t **p_report_map_data,
                                 uint16_t *report_map_size,
                                 uint8_t report_item_info) {
...
static const HIDUsage parse_usage(uint8_t item_info, uint32_t data,
                                  uint16_t usage_page) {
  if ((item_info & HID_ITEM_SIZE_MASK) == HID_ITEM_SIZE_32) {
    ESP_LOGD(TAG, "Parsing extedned usage: %X, %X", (uint16_t)(data >> 16),
             (uint16_t)data);
...
const HIDUsage HIDUsageList::get_usage(uint16_t index) const {
  ESP_LOGD(TAG, "get usage for index %d with list size %d", index,
           this->usages.size());
...
HIDReportMap *
HIDReportMap::parse_report_map_data(const uint8_t *report_map_data,
                                    uint16_t report_map_size) {
  ...
  while (report_map_size) {
    ...
    uint32_t report_item_data = HIDReportMap::parse_item(
        &report_map_data, &report_map_size, report_item_info);
    ...
    case HID_ITEM_TYPE_TAG_REPORT_ID: {
      if (input_reports.count(report_item_data) == 0) {
        input_reports.emplace(report_item_data,
                              new HIDInputReport(report_item_data));
      }
    ...
    case HID_ITEM_TYPE_TAG_USAGE: {
      usages.push_back(parse_usage(report_item_info, report_item_data,
                                   state_table.usage_page));
      break;
    }

    case HID_ITEM_TYPE_TAG_USAGE_MINIMUM: {
      usage_range.minimum = parse_usage(report_item_info, report_item_data,
                                        state_table.usage_page);
      break;
    }

    case HID_ITEM_TYPE_TAG_USAGE_MAXIMUM: {
      usage_range.maximum = parse_usage(report_item_info, report_item_data,
                                        state_table.usage_page);
      break;
    }
...

    {
      ...
      if (usages.size() > 0) {
        ...
        ESP_LOGD(TAG, "Creating usage range with min: %d, max: %d, page: %d",
                 usage_range.minimum.usage, usage_range.maximum.usage,
                 usage_range.minimum.page);
        usage_collection = new HIDUsageRange(
            usage_range.minimum, usage_range.maximum, usage_range.minimum.page);
      }
      if (item_flags & HID_IOF_VARIABLE) {
        input_report->push_back(new HIDInputReportVariable(
            usage_collection, state_table.report_count, state_table.report_id,
            state_table.logical_range, state_table.report_size,
            input_report->get_next_offset()));
        ESP_LOGD(TAG,
                 "Parsed input report item of type: variable, report size: %d, "
                 "report count: %d, report id: %d",
                 state_table.report_size, state_table.report_count,
                 state_table.report_id);
      } else {
        input_report->push_back(new HIDInputReportArray(
            usage_collection, state_table.report_count, state_table.report_id,
            state_table.logical_range, state_table.report_size,
            input_report->get_next_offset()));
        ESP_LOGD(TAG,
                 "Parsed input report item of type: array, report size: %d, "
                 "report count: %d, report id: %d",
                 state_table.report_size, state_table.report_count,
                 state_table.report_id);
      }
  ...
  ESP_LOGD(TAG, "Parsed report map with %d input reports",
           input_reports.size());
  return report_map;
}

uint8_t HIDInputReport::get_next_offset() { return this->report_size; }
...
std::vector<HIDReportItemValue> HIDInputReport::parse(uint8_t *report_data) {
  ...
  for (HIDInputReportItem *report_item : this->items) {
    std::vector<HIDReportItemValue> item_values =
        report_item->parse(report_data);
...
int32_t HIDInputReportItem::parse_input_report_item(
    uint8_t *report_data, uint16_t bit_offset, uint16_t report_size,
...
std::string HIDReportItemValue::to_string() const {
  return vformat("HIDReportItemValue(usage_page: %d, usage: %d, value: %d)",
                 this->usage.page, this->usage.usage, this->value);
}

std::vector<HIDReportItemValue>
HIDInputReportVariable::parse(uint8_t *report_data) {
  std::vector<HIDReportItemValue> values;
  for (uint8_t i = 0; i < this->report_count; i++) {
    int32_t value = parse_input_report_item(
        report_data, this->report_offset + i * this->report_size,
        this->report_size, this->logical_range);
    if (value > this->logical_range.maximum ||
        value < this->logical_range.minimum) {
    ...
    if (this->last_values[i].raw_value == value)
      continue;
    values.push_back(
        HIDReportItemValue(this->usage_collection->get_usage(i), value, value));
...
std::vector<HIDReportItemValue>
HIDInputReportArray::parse(uint8_t *report_data) {
  std::vector<HIDReportItemValue> values = {};
  ...
  for (uint8_t i = 0; i < this->report_count; i++) {
    ...
    if (value == 0) {
      if (this->last_values[i].value) {
        values.push_back(
            HIDReportItemValue(this->last_values[i].usage, 0, value));
        last_values[i] = values.back();
        ESP_LOGD(TAG, values.back().to_string().c_str());
      }
    } else {
      if (this->last_values[i].value == 0) {
        values.push_back(HIDReportItemValue(
            this->usage_collection->get_usage(value), 1, value));
        last_values[i] = values.back();
